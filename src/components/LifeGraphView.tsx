...
export type ColorMode = "vivid" | "mono";
...
const VIVID_LAYER_COLOR: Record<string, string> = {
    foundation:     "#55cdfc",
...
const VIVID_STATUS_COLOR: Record<string, string> = {
    active: "#55cdfc", ongoing: "#a8e6cf", soon: "#ff9de2", later: "rgba(255,255,255,0.3)",
};
const VIVID_STATUS_DOT: Record<string, string> = {
...
function nodeColor(node: GraphNodeInput, colorMode: ColorMode): string {
    if (colorMode === "mono") return "#cccccc";
    return (node.layer && VIVID_LAYER_COLOR[node.layer]) || "#ffffff";
}
...
const LifeGraphView: React.FC<LifeGraphViewProps> = ({
    ...
    // Shared nodes ref for the banner RAF (avoids stale closure)
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            edges.forEach((e, ei) => {
                ...
                const rgbA = colorMode==="mono" ? "200,200,200" : hexToRgb(nodeColor(na, colorMode));
                const rgbB = colorMode==="mono" ? "200,200,200" : hexToRgb(nodeColor(nb, colorMode));
                ...
                const mx=(a.px+b.px)/2+(b.py-a.py)*0.08;
                const my=(a.py+b.py)/2-(b.px-a.px)*0.08;
            ...
            drawOrder.forEach(node => {
                ...
                const col   = nodeColor(node, colorMode);
                ...
                if (d<nr+12 && d<minDist) { minDist=d; newHovered=node; }
                ...
                // Status dot (vivid only)
                if (colorMode==="vivid" && node.status) {
                    ctx.beginPath();
                    ctx.arc(p.px+nr*0.62,p.py-nr*0.62,Math.max(2,2.8*p.scale),0,Math.PI*2);
                    ctx.fillStyle=VIVID_STATUS_DOT[node.status]||"#444"; ctx.fill();
                }
                ...
                if (node) {
                    if (node.id !== lastId) {
                        ...
                        if (hbTitleRef.current) {
                            ...
                            hbTitleRef.current.style.color = nodeColor(node, colorMode);
                        }
                        ...
                        if (hbStatusRef.current) {
                            ...
                            hbStatusRef.current.style.color = node.status
                                ? (colorMode==="vivid" ? (VIVID_STATUS_COLOR[node.status]||"rgba(255,255,255,0.3)") : "rgba(255,255,255,0.5)")
                                : "";
                        }
                        if (hbEdgesRef.current) {
                            ...
                            edges.filter(e=>e.s===node.id||e.t===node.id).forEach(e => {
                                ...
                                chip.innerHTML = `<span class="lgv-ea">${arrow}</span><span class="lgv-el" style="color:${otherColor}">${otherLabel}</span><span class="lgv-et">${typeLabel}</span>`;
    ...
    return (
        <div style={{ position:"fixed", inset:0, zIndex:9999, background:"#0a0a0f", fontFamily:"'DM Mono',monospace", overflow:"hidden" }}>
            ...
            {/* Footer legend */}
            <div style={{ position:"absolute", bottom:0, left:0, right:0, display:"flex", alignItems:"center", background:"rgba(10,10,15,.88)", borderTop:"1px solid rgba(255,255,255,.08)", backdropFilter:"blur(14px)", padding:"0 28px", height:44, pointerEvents:"none" }}>
                {colorMode==="vivid" && (
                    <div style={{ display:"flex", alignItems:"center", gap:14, paddingRight:20, marginRight:20, height:"100%", borderRight:"1px solid rgba(255,255,255,.07)", flexShrink:0 }}>
                        {Object.entries(VIVID_LAYER_COLOR).map(([layer, color]) => (
                ...
                <div style={{ display:"flex", alignItems:"center", gap:14, flexShrink:0 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:9, color:"rgba(255,255,255,.35)", letterSpacing:"0.07em", textTransform:"uppercase", whiteSpace:"nowrap" }}>
                        <div style={{ width:20, height:1, background:"rgba(255,255,255,.4)", flexShrink:0 }}/>prerequisite
                    </div>
                    <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:9, color:"rgba(255,255,255,.35)", letterSpacing:"0.07em", textTransform:"uppercase", whiteSpace:"nowrap" }}>
                        <div style={{ width:20, height:1, borderTop:"1px dashed rgba(255,255,255,.25)", flexShrink:0 }}/>association
                    </div>
                </div>
            </div>

            {/* Edit button — only when onEdit provided, hidden while hovering */}
            {onEdit && (
                <button
                    ref={editBtnRef}
                    onClick={onEdit}
