...
import { Project, Idea, Task, Domain } from "../lib/mockData";
...
function findSimilarProject(projectName: string, projects: Project[]): Project | null {
...
const stateMutationService: StateMutationService = {
    applyClassifications(
        ...
        for (const classification of classifications) {
            const { domain, project, idea, tasks, shouldCreateDomain } = classification;
            ...
            if (project?.matchExisting !== undefined) {
                ...
                targetProject = newProjects.find(p => p.id === project.matchExisting);
            ...
            if (targetProject && idea?.name) {
                ...
                const similarIdea = findSimilarIdea(idea.name, targetProject.ideas);
                ...
                if (!similarIdea) {
                    ...
                    const existingTaskNames = new Set(similarIdea.tasks.map(t => t.name.toLowerCase()));
                    const newTasks = (tasks || [])
                        .filter(t => !existingTaskNames.has(t.name.toLowerCase()))
                        .map(t => ({
                            id: generateId(),
                            name: t.name,
                            is_completed: false,
